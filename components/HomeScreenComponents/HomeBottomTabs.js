...
import {getEvents, insertEvent} from '../../database/database';
...
const HomeBottomTabs = ({scanDocument, setTab, tab, setEvents}) => {
  const handleAddDocs = () => {
    scanDocument();
    setTab('add');
  };
  ...
  const handleSetTab = async tab => {
    setTab(tab);
    await insertEvent('ButtonClick', {button: tab});
      ...
      <View style={tw`flex flex-row w-1/3`}>
        ...
        <TouchableOpacity
          ...
          <View style={tw`flex items-center justify-center gap-2`}>
            ...
            <Text style={tw`${tab == 'warranty' && 'text-white'} text-xs`}>
              Warranty
            </Text>
          </View>
        </TouchableOpacity>
